...
const HomePage = () => {
  ...
  const getAllTask = async ()=>{
    try{
       ...
       setLoading(true)
       const res = await axios.post('https://todo-backend-pxuk.onrender.com/api/v1/tasks/get-task',{userid:user._id})
       setLoading(false)
       setAllTask(res.data)
  ...
  const handleSubmit = async (values) =>{
      try{
         ...
         setLoading(true)
         if(editable){
          await axios.post('https://todo-backend-pxuk.onrender.com/api/v1/tasks/edit-task',{payload:{
            ...values,
            userId:user._id
          },taskId:editable._id
        }) ;
